...
app = Flask(__name__)
# CORS(app, resources={r"/api/chatbot/*": {"origins": "https://www.pkbookings.com"}})
CORS(app)
...
GEMINI_API_KEY = "AIzaSyBzVbcyVkFvPl8YC067Rhla-JqOkCV3OLE"
os.environ["GEMINI_API_KEY"] = GEMINI_API_KEY
genai.configure(api_key=os.environ["GEMINI_API_KEY"])

# AIzaSyDCJ5mT9ofMgFp1fiHhZmJ8-5bTDBktxVg
...
model = genai.GenerativeModel(
    ...
    system_instruction="Your name is SmartBot, your virtual assistant! and you are ready to assist with their queries under 150 words. If you do not know the answer to a question, suggest that they try rephrasing their prompt or give the user contact information so that we can contact. "+system_instruction  # Use extracted content here
)
...
@app.route('/')
def home():
    return render_template('index1.html')  # Serve the HTML file
...
def get_response():
    ...
    try:
        ...
        question = data['text']
        app.logger.debug(f"User question: {question}")
        ...
        app.logger.debug(f"Response from AI model: {response}")
